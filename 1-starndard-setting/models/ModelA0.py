...
class Tunnel(chainer.Chain):
	...
	def __init__(self, dim, dropout_rate, activate, layer, isR, isBN, relation_size, pooling_method):
		...
		# transition function for head entities that are neighbor of the query entity
		linksH = [('h{}'.format(i), Block(dim, dropout_rate, activate, layer, isR, isBN)) for i in range(relation_size)]
		for link in linksH:
			self.add_link(*link)
		self.forwardH = linksH

		# transition function for tail entities that are neighbors of the query entity
	...
	Google Translate: There is no edge coming out of the source entity
	"""
...
class Model(chainer.Chain):
	def __init__(self, args):
		...
		linksB = [('b{}'.format(i),
				   Tunnel(args.dim, args.dropout_block, args.activate, args.layerR, args.is_residual, args.is_batchnorm,
						  args.rel_size, args.pooling_method)) for i in range(args.order)]
		for link in linksB:
			self.add_link(*link)
		...
		for i, e in enumerate(entities):
			...
			if e in links:
				...
				if len(edges[e]) <= self.sample_size:
					...
					nn = random.sample(edges[e], self.sample_size)
				if len(nn) == 0:
					print('something wrong @ modelS')
