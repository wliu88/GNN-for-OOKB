...
# gold_heads is a dictionary mapping from (relation, tail) to a set of head entities, from train
gold_heads = defaultdict(set)
gold_tails = defaultdict(set)
# gold_relations is a dictionary mapping from (head, tail) to relation, from both train and aux
gold_relations = dict()

# tail_per_head is a dictionary mapping from head to its number of tail neighbors, from train
tail_per_head = defaultdict(set)
head_per_tail = defaultdict(set)

# candidate_heads is a dictionary mapping from a relation to a list of head entities, from train
candidate_heads = defaultdict(set)
candidate_tails = defaultdict(set)

# train_link is a dictionary from one entity to a list of its neighboring entities, from train
...
def main(args):
    ...
    xp = Backend(args)
    m = model.Model(args)
    # set up optimizer according to parameters in args
    opt = get_opt(args)
    opt.setup(m)
